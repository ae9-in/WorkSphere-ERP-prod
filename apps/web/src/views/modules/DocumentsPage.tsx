    ...
    >
      ...
      {activeTab === 'dashboard' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-4">
            {[
              { title: 'Total Documents',    value: documents.length,        icon: <FileText size={20} />,    bg: 'bg-ag-primary-light text-ag-primary' },
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Total Documents',   val: documents.length,           color: 'text-ag-primary bg-ag-primary-light' },
              { label: 'Verification Rate', val: documents.length > 0 ? `${Math.round((verifiedCount / documents.length) * 100)}%` : '0%', color: 'text-ag-mint bg-[#E6FAF4]' },
              { label: 'Compliance Rate',   val: employees.length > 0 ? `${Math.round((verifiedCount / Math.max(1, employees.length * EMPLOYEE_CATEGORIES.reduce((s, c) => s + c.docs.filter(d => d.required).length, 0))) * 100)}%` : '0%', color: 'text-ag-sky bg-[#E8F6FF]' },
              { label: 'Storage Used',      val: formatFileSize(totalSize),  color: 'text-ag-amber bg-[#FFF8E6]' },
            ].map(k => (
              <Card key={k.label} className="p-4 flex items-center gap-3">
                <div className={`p-2.5 rounded-xl ${k.color} shrink-0`}><Database size={20} /></div>
                <div>
                  <span className="text-[9px] font-black uppercase tracking-wider text-ag-ink-3 block">{k.label}</span>
