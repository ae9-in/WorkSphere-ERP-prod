...
export default function SupplyChainPage() {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'shipments' | 'dispatch' | 'fleet' | 'tracking' | 'returns'>('dashboard');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [
        ...
        analyticList, trackList, costList, optList, repList
      ] = await Promise.all([
              ...
              <div className="p-6 space-y-4">
                {shipments.filter(s => s.status === 'dispatched' || s.status === 'in_transit').map((s) => (
                  <div key={s._id} className={`p-4 border rounded-xl flex justify-between items-center ${selectedShipment?._id === s._id ? 'border-ag-primary bg-ag-primary/5' : 'border-ag-border bg-ag-surface'}`}>
                    ...
                    <div className="flex gap-2">
                      <Button variant="secondary" size="sm" onClick={() => setSelectedShipment(s)}>
                        Track / POD
                      </Button>
                    </div>
                  </div>
